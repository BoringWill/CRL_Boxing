...
def add_identity_channel(obs, num_envs):
    obs[:, 4:, :, :] = 0.0
    obs[0::2, 4, :, :] = 255.0  # P0 (白色)
    obs[1::2, 5, :, :] = 255.0  # P1 (黑色)
    return obs
...
def train():
    ...
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed - local_rank}__{timestamp}"
    ...
    if args.load_model_path and os.path.exists(args.load_model_path):
        try:
            ...
            if local_rank == 0: print(f"--> [Success] 加载权重: {args.load_model_path}")
        except Exception as e:
            if local_rank == 0: print(f"--> [Warning] 加载权重失败: {e}")

    initial_step = global_step
    if local_rank == 0 and not os.path.exists(opponent_model_path):
    ...
    current_opp_probs = [0.0 for _ in range(num_games)]  # 🚨 OpenAI 策略：记录采样概率 p_i

    def select_new_opponent(game_idx):
        if len(opponent_pool_paths) > 0 and random.random() < HISTORICAL_RATIO:
            # OpenAI 策略：Softmax 采样 $p_i \propto \exp(q_i)$
            ...
            current_opp_probs[game_idx] = final_probs[idx]  # 记录 p_i
        else:
    ...
    # --- 5. Buffer 初始化 ---
    num_total_agents_local = args.num_envs * 2
    obs_shape = (6, 84, 84)

    obs = torch.zeros((args.num_steps, num_total_agents_local) + obs_shape).to(device)
    actions = torch.zeros((args.num_steps, num_total_agents_local) + envs.single_action_space.shape).to(device)
    logprobs = torch.zeros((args.num_steps, num_total_agents_local)).to(device)
    rewards = torch.zeros((args.num_steps, num_total_agents_local)).to(device)
    dones = torch.zeros((args.num_steps, num_total_agents_local)).to(device)
    values = torch.zeros((args.num_steps, num_total_agents_local)).to(device)
    student_masks = torch.zeros((args.num_steps, num_total_agents_local), dtype=torch.bool).to(device)
    ...
    next_done = torch.zeros(num_total_agents_local).to(device)
    ...
    num_updates = args.total_timesteps // (args.num_steps * num_total_agents_local * world_size)
    start_update = (global_step // (args.num_steps * num_total_agents_local * world_size)) + 1
    ...
    for update in range(start_update, num_updates + 1):
        ...
        for step in range(args.num_steps):
            global_step += num_total_agents_local * world_size
            dones[step] = next_done

            curr_mask = torch.zeros(num_total_agents_local, dtype=torch.bool, device=device)
            ...
            if args.capture_video and local_rank == 0:
                ...
                if n_term[2 * k] or n_trunc[2 * k]:
                    ...
                    if score != 0:
                        ...
                        if student_is_p0[k]:
                            ...
                            if local_rank == 0: writer.add_scalar("charts/win_rate_student_P0_white",
                                                                  np.mean(recent_wins_p0), global_step)
                        else:
                            recent_wins_p1.append(win)
                            if local_rank == 0: writer.add_scalar("charts/win_rate_student_P1_black",
                                                                  np.mean(recent_wins_p1), global_step)

                        # 🚨 OpenAI 策略：更新池分 $q_i \leftarrow q_i - \frac{\eta}{N p_i}$
                    ...
                    if local_rank == 0: writer.add_scalar("charts/student_episodic_return", score, global_step)
        ...
        evolve_tensor = torch.tensor(local_can_evolve, device=device)
        if world_size > 1:
            dist.all_reduce(evolve_tensor, op=dist.ReduceOp.MAX)
        global_can_evolve = evolve_tensor.item() > 0

        if global_can_evolve:
            evolution_trigger_count += 1
            if local_rank == 0:
                print(
                    f"\n[Evolution] 触发！当前卡胜率: {total_win_rate:.1%} (P0:{p0_win_rate:.1%}, P1:{p1_win_rate:.1%})")
        ...
        if local_rank == 0:
            ...
            writer.add_scalar("charts/SPS", actual_sps, global_step)

            # 🚨 OpenAI 策略：额外监控每个历史模型的池分
